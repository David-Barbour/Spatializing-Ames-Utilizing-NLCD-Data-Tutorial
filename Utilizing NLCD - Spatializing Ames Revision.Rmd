...
Shrub/Scrub 300m has some NA values and some zeros, the source of this issue is that the landcover only occurs in the collum,n twice in the 0607 year prior to the rowbind and misses our zeroing out code. Correcting it here
```{r}
combined_houses$`Shrub/Scrub 300m`[is.na(combined_houses$`Shrub/Scrub 300m`)] <- 0
```
...
calculate_aggregation_index <- function(raster_input, buffer_sf) {
  ...
  result <- sample_lsm(
    ...
    directions = 8             # Consider 8 neighboring cells for adjacency (queen’s case)
  )
